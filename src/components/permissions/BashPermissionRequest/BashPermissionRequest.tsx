...
function BashPermissionRequestInner({
  ...
  return <PermissionDialog workerBadge={workerBadge} title={sandboxingEnabled_0 && !isSandboxed_0 ? 'Comando Bash (sem sandbox)' : 'Comando Bash'} subtitle={classifierSubtitle}>
      ...
      {showPermissionDebug ? <>
          ...
          <Box justifyContent="space-between" marginTop={1}>
            <Text dimColor>
              Esc pra cancelar
              {(focusedOption === 'yes' && !yesInputMode || focusedOption === 'no' && !noInputMode) && ' · Tab pra complementar sua resposta'}
              {explainerState.enabled && ` · ctrl+e pra ${explainerState.visible ? 'esconder' : 'explicar'}`}
            </Text>
            {toolUseContext.options.debug && <Text dimColor>Ctrl+d pra mostrar info de debug</Text>}
          </Box>
        </>}
    </PermissionDialog>;
}
