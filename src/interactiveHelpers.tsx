...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    // Show session picker dialog if there are previous sessions and not resuming/continuing.
    // Only shown on fresh starts (no --resume/--continue flags).
    const hasPreviousSessionsCheck = process.argv.slice(2);
    if (!hasPreviousSessionsCheck.includes('--resume') && !hasPreviousSessionsCheck.includes('--continue')) {
      let selectedSessionId: string | null = null;
      await showSetupDialog<void>(root, done => {
        return React.createElement(require('./components/SessionPickerDialog.js').SessionPickerDialog, {
          onNew: () => { done(undefined); },
          onResume: (sessionId: string) => { selectedSessionId = sessionId; done(undefined); },
          onSkip: () => { done(undefined); },
        });
      }, { onChangeAppState });
