...
export async function teleportResumeCodeSession(sessionId: string, onProgress?: TeleportProgressCallback): Promise<TeleportRemoteResponse> {
  ...
  try {
    ...
    if (!accessToken) {
      ...
      throw new Error('Sessões web do Claudinho requerem autenticação com uma conta Claude.ai. Autenticação por API key não é suficiente. Execute /login pra autenticar, ou verifique seu status de autenticação com /status.');
    }
