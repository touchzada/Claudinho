 ...
 * Anthropic OAuth client. It manages:
...
// ── PKCE helpers (reuse from existing OAuth) ──────────────────────────────────
...
/**
 * Generates the code challenge from a verifier using SHA-256.
 */
...
function generateState(): string {
  ...
  crypto.getRandomValues(array)
  return Buffer.from(array).toString('base64url').replace(/=/g, '')
}
 ...
 */
export async function buildCodexAuthUrl(): Promise<{
  url: string
  verifier: string
  state: string
}> {
  ...
  url.searchParams.set('originator', 'claudinho')
...
export async function runCodexOAuthFlow(
  ...
  const { url, verifier, state } = await buildCodexAuthUrl()
