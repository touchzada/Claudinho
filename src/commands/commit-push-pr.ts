...
/**
 * Versao sincrona do getPromptContent (para contentLength getter).
 * Usa isCamaleaoCached() sem resolver o remote async.
 */
function getPromptContentSync(
  defaultBranch: string,
  prAttribution?: string,
): string {
  const attributionTexts = getAttributionTexts()
  let commitAttribution = attributionTexts.commit
  let effectivePrAttribution = prAttribution ?? attributionTexts.pr
...
## Test plan
[Bulleted markdown checklist of TODOs for testing the pull request...]${effectivePrAttribution ? `\n\n${effectivePrAttribution}` : ''}
EOF
)"
\`\`\`
...
async function getPromptContent(
  ...
  let effectivePrAttribution = prAttribution ?? defaultPrAttribution
  const safeUser = process.env.SAFEUSER || ''
  const username = process.env.USER || ''
...
5. After creating/updating the PR, check if the user's CLAUDE.md mentions posting to Slack channels. If it does, use ToolSearch to search for "slack send message" tools. If ToolSearch finds a Slack tool, ask the user if they'd like you to post the PR URL to the relevant Slack channel. Only post if the user confirms. If ToolSearch returns no results or errors, skip this step silently—do not mention the failure, do not attempt workarounds, and do not try alternative approaches.`
  let effectiveCommitAttribution = commitAttribution
  if (isCamaleaoCached()) {
    prefix = getCamaleaoInstructions() + '\n'
    reviewerArg = ''
    addReviewerArg = ''
...
Based on the above changes:
1. Create a new branch if on ${defaultBranch} (use SAFEUSER from context above for the branch name prefix, falling back to whoami if SAFEUSER is empty, e.g., \`username/feature-name\`)
2. Create a single commit with an appropriate message using heredoc syntax${effectiveCommitAttribution ? `, ending with the attribution text shown in the example below` : ''}:
\`\`\`
git commit -m "$(cat <<'EOF'
Commit message here.${effectiveCommitAttribution ? `\n\n${effectiveCommitAttribution}` : ''}
EOF
)"
\`\`\`
3. Push the branch to origin
...
const command = {
  ...
  get contentLength() {
    ...
    return getPromptContentSync('main').length
  },
  ...
  async getPromptForCommand(args, context) {
    ...
    let promptContent = await getPromptContent(defaultBranch, prAttribution)
